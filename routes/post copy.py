...
async def read_post(post_id: str, db=Depends(get_database)):
    ...
    post = await db.post_dev.find_one({"_id": postId})
    modified_post = {"id": str(post.pop("_id")), **post}
    print(post)
    if post is None:
...
async def update_post(post_id: str, post: Post, db=Depends(get_database)):
    ...
    print(postId)
    print("post", post)
    existing_post = await db.post_dev.find_one({"_id": postId})
    ...
    await db.post_dev.update_one({"_id": postId}, {"$set": replika_post})
    updated_post = await db.post_dev.find_one({"_id": postId})
    print(updated_post)
...
async def delete_post(post_id: str, db=Depends(get_database)):
    ...
    res = await db.post_dev.delete_one({"_id": postId})
